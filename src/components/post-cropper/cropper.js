...
export default {
  ...
  computed: {
    transformMeta: function() {
      ...
      return `translate3d(${transform.translate.x}px, ${transform.translate.y}px, 0) scale(${transform.zoom})`
    },
    ctrlStyle: function() {
  ...
  methods: {
    ...
    resetCutReact() {// init size and position of the cutter
      ...
      this.ctrlWidth = this.containerWidth;
      this.ctrlHeight = this.containerHeight;
    ...
    async initImage() {
      ...
      // this.image.scale = err ? 1 : res.height / res.width;
    ...
    setBoundary() {
      ...
      if(zoom < this.minZoom){
        zoom = this.minZoom;
        // this.initImage();
        let nowWidth = this.ctrlWidth;
        let nowHeight = this.image.originHeight / this.image.originWidth * this.ctrlWidth;
        this.image.width = nowWidth;
        this.image.height = nowHeight;

        console.log("--------------1");

      }else if(zoom >= this.maxZoom){
        zoom = this.maxZoom;
        console.log("--------------2", this.maxZoom);
      }else{
        zoom = zoom;
        console.log("--------------3");
      }

      // console.log('this.image.height',this.ctrlHeight);
      ...
      if(this.image.width * zoom >= this.ctrlWidth){
        translate.x = Math.floor(translate.x < minX ? minX : (translate.x > maxX ? maxX : translate.x))
        console.log("--------------4");
      }

      if(this.image.height * zoom >= this.ctrlHeight){
    ...
    draw() {
      ...
      let realWidth = this.image.width >= this.ctrlWidth ? this.ctrlWidth * canvasZoom : this.image.width * canvasZoom;
      let realHeight = this.image.height >= this.ctrlHeight ? this.ctrlHeight * canvasZoom : this.image.height * canvasZoom;
